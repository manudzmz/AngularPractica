...
export class PostService {
    ...
    getUserPosts(id: number): Observable<Post[]> {
        ...
        let fecha_actual = new Date().getTime();
        return this._http
                   .get(`${this._backendUri}/posts?author.id=${id}&publicationDate_lte=${fecha_actual}&_sort=publicationDate&_order=DESC`)
                   .map((response: Response) => Post.fromJsonToList(response.json()));
    }
    ...
    createPost(post: Post): Observable<Post> {
        ...
        /*----------------------------------------------------------------------------------|
         ...
         | 'fromJson() para crear un nuevo objeto Post basado en la respuesta HTTP obtenida. |
         |----------------------------------------------------------------------------------*/

        return this._http
                        .post(`${this._backendUri}/posts`, post)
                        .map((respuesta: Response) => {
                            return Post.fromJson(respuesta.json)
                        });
    }
}
